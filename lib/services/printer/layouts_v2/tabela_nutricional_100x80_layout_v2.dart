...
class TabelaNutricional100x80LayoutV2 {
  void build({
    ...
    final col2 = tableLeft + (tableWidth * 0.52).toInt();
    final col3 = tableLeft + (tableWidth * 0.70).toInt();
    final col4 = tableLeft + (tableWidth * 0.89).toInt();
    ...
    w.bar(x: col2 - 10, y: verticalTop, width: 1, height: verticalHeight);
    w.bar(x: col3 - 10, y: verticalTop, width: 1, height: verticalHeight);
    w.bar(x: col4 - 10, y: verticalTop, width: 1, height: verticalHeight);
  ...
  int _printNutriRow({
    ...
    bool usarTracoQuandoZero = false,
  }) {
    ...
    w.text(
      ...
      y: y,
      spec: const TsplFontSpec(font: '1', xMul: 1, yMul: 1),
      text: (usarTracoQuandoZero && vd <= 0) ? '-' : '${vd.round()}%',
    );
